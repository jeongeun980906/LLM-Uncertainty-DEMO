...
import sys
from model.util import (compute_view_params, get_rotation_matrix_from_two_points,
...
class MuJoCoParserClass(object):
    ...
    def set_office_scene(self):
        ...
        jntadr_starbucks = self.model.body('starbucks_cup').jntadr[0]
        self.model.joint(jntadr_starbucks).qpos0[:3] = np.array([0.78,0,0.79])

        jntadr_iphone_14_pro = self.model.body('iphone_14_pro').jntadr[0]
        self.model.joint(jntadr_iphone_14_pro).qpos0[:3] = np.array([0.68,-0.1,0.79])
